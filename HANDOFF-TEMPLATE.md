...
### 2 · Edit sequence (shot by shot)
A numbered table: `# | clip (real IMG/video filename) | role + what's on screen | on-screen text | ~seconds`. Cold-open first (the hook / strongest reveal), craft/middle beats, payoff, end card/CTA. Call out b-roll/cutaway clips and an explicit DO-NOT-USE list (receipts, dupes, unusable).

### 3 · Voiceover script
Numbered lines in her voice, each mapped to a shot ("(over 3-4)"). Short, calm, a little secret. No em-dashes. This is what Kailin records. If a line depends on footage/audio Margaux couldn't confirm, mark it a suggestion, never assert invented dialogue.
